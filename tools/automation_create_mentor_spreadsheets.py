...
def process_mentees(file_path, sheet_name, output_dir):
    """Processes the mentee registration data and generates Excel files for each mentor."""
        ...
        if pd.notna(row[mentor_column]):  # Check if the mentor column is not empty
            ...
            for entry in mentor_entries:
                ...
                if match:
                    ...
                    # Normalize mentor names to avoid variations
                    mentor_name = mentor_name.lower().strip().title()  # Convert to Title Case for consistency
    ...
    print("Files created successfully in 'Long Term Mentors' folder!")  # Confirmation message

if __name__ == "__main__":
    # Set file paths

    file_path = "samples/Mentorship Programme long-term Registration Form for Mentees (Responses).xlsx"
    sheet_name = "Revised Mentees"
    output_dir = "Long Term Mentors" #change this to local folder path

    # Run the mentor processing logic
    process_mentees(file_path, sheet_name, output_dir)  # Call your main function (assuming you structured the script properly)